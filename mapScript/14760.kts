...
val badplayernumber = ArrayList<Int>()
val goodplayer = ArrayList<Player>()
val goodplayeruuid = ArrayList<String>()
val badplayeruuid = ArrayList<String>()
val haveplayer = ArrayList<Player>()
val rangeplayerlist = ArrayList<Player>()
...
fun LaunchPadBuild.removeZT(amount: Int) { LaunchPadZT.put(this, ZT()- ZT() - amount) }
fun LaunchPadBuild.addZT(amount: Int) { LaunchPadZT.put(this, ZT() - ZT() + amount)}
fun taskcount(): Int{
    var i = 0
...
fun Player.setplayervoted(){
    return playervoted.put(uuid(),playervoted.get(uuid())-playervoted.get(uuid()))
}
    ...
    menu.sendMenuBuilder<Unit>(
        ...
        if(Launchpad.ZT() != 1) {
            if (Launchpad.ON() <= 0) {
                this += listOf(
                    "[green]确定" to {
                        unit().apply(StatusEffects.unmoving, 800f)
                        ...
                        while (true) {
                            if (Time.millis() - lastTime >= 10_000) {
                                ...
                                for (allplayer in Groups.player) {
                                    allplayer.sendMessage("位于${((Launchpad.x)/8f).toInt()},${((Launchpad.x)/8f).toInt()}的任务点被修复")
                                }
                    ...
                    "[green]确定" to {
                        ...
                        while (true) {
                            if (Time.millis() - lastTime >= 3_000) {
                                ...
                                for (allplayer in Groups.player) {
                                    allplayer.sendMessage("位于${((Launchpad.x)/8f).toInt()},${((Launchpad.x)/8f).toInt()}的任务点被破坏")
                                }
                                Launchpad.removeZT(1)
                                break
                            }
    ...
    menu.sendMenuBuilder<Unit>(
        ...
        add(listOf(
            "[green]确定" to {
                ...
                for(allplayer in Groups.player){
                    allplayer.sendMessage("${name}发起了紧急投票")
                }
            }, "[red]否" to {
            }
        ))
...
fun Player.destorydelay(){
    launch(Dispatchers.game) {
        ...
        if (getdestory() >= 1) {
            delay(100)
            sendMessage("[red]猎杀破坏技能进入冷却20s后冷却完毕")
            while (true) {
                if (Time.millis() - lastTime >= 20_000) {
                    adddestory(-1)
...
fun Player.killdelay(){
    launch(Dispatchers.game){
        ...
        if(getkills()>=1) {
            delay(100)
            sendMessage("[red]猎杀技能进入冷却20s后冷却完毕")
            while (true) {
                if (Time.millis() - lastTime >= 20_000) {
                    addkills(-1)
        ...
        while(true){
            for (allplayer in Groups.player) {
                ...
                Groups.player.forEach {
                    if (it.x / 8 >= 220f) it.unit().set(180f*8f, it.y)
                    if (it.x / 8 <= 180f) it.unit().set(220f*8f, it.y)
                    if (it.y / 8 >= 220f) it.unit().set(it.x, 180f*8f)
                    if (it.y / 8 <= 180f) it.unit().set(it.x, 220f*8f)
                }
                delay(20)
            }
        }
...
onEnable{
    launch{
        delay(100)
        if(Groups.player.count() < 2) return@launch
        loop(Dispatchers.game) {
            val lastTime = Time.millis()
            delay(20000)
            while (true) {
                Groups.player.forEach {allplayer->
                    Call.setHudText(allplayer.con, "[yellow]点击自己报警以开始快速投票\n[green]距离下一次自动投票开始还有:${(300 * 1000 - (Time.millis() - lastTime)) / 1000}/s")
                    for(allplayer in Groups.player) {
                ...
                delay(100)
                if(Time.millis()-lastTime>=300_000||Start == true) {
                    VoteStart = true
                    for(allplayer in Groups.player){
                        allplayer.addplayerchance(1)
                        allplayer.sendMessage("你已经获得一次紧急投票机会")
                    }
                    voteStrat()
                    Groups.player.forEach { allplayer ->
                    ...
                    while (true) {
                        Groups.player.forEach {allplayer->
                            Call.setHudText(allplayer.con, "[yellow]群体玩家使用投票可以快速通过\n[yellow]点击其他玩家投票\n[yellow]点击核心弃票\n[red]距离投票结束还有:${(80 * 1000 - (Time.millis() - lastTime)) / 1000}/s")
                        ...
                        for(allplayer in Groups.player) {
                            var amount = allplayer.getplayervoted()
                            var x = allplayer.unit().x
                            var y = allplayer.unit().y
                            playerui(amount, x, y)
                        }
...
onEnable {
    launch {
        ...
        for(allplayer in Groups.player) {
            allplayer.setplayer()
            allplayer.addplayernumber(0)
            ChangeTeam(allplayer,Team.get(1))
        }
        badplayer.clear()
        goodplayer.clear()
        badplayernumber.clear()
        Groups.player.forEach { allplayer->
            if(Groups.player.count() >= 4)
                allplayer.sendMessage("[red]游戏即将开始")
            else
                allplayer.sendMessage("[red]游戏人数不足")
        }
        delay(20000)
        var i = 1
        val lastTime =  Time.millis()
...
listen<EventType.PlayerJoin>{
    //if(Groups.player.count() < 2) return@listen
    val player = it.player
    for(allplayer in goodplayer){
        goodplayeruuid.add(allplayer.uuid())
    }
    for(allplayer in badplayer){
        badplayeruuid.add(allplayer.uuid())
    }
    if(player.uuid() !in badplayeruuid && player.uuid() !in goodplayeruuid) {
        player.sendMessage("你为后来加入者无法参加这次游戏")
        player.removeplayer(1)
        ChangeTeam(player, Team.get(255))
    }
    badplayeruuid.clear()
    goodplayeruuid.clear()
}
...
listen<EventType.PlayerLeave>{
    launch {
        ...
        val player = it.player
        Groups.player.forEach { allplayer ->
            allplayer.sendMessage("[red]玩家${player.getplayernumber()}离开如果该玩家不在60秒后回来将会按照移除该玩家")
        }
        val lastTime = Time.millis()
        while (true) {
            if (Time.millis() - lastTime >= 60_000) {
                if (player in badplayer)
                    badplayer.remove(player)
                else {
                    if (player in goodplayer)
                        goodplayer.remove(player)
                }
            }
            listen<EventType.PlayerJoin> {
                val newplayer = it.player
                if (newplayer.uuid() == player.uuid()) return@listen
            }
            delay(100)
            badplayeruuid.clear()
            goodplayeruuid.clear()
        ...
        if (player.team() == Team.get(255)) {
            ...
            for(player255 in Groups.player){
                if(player255.team() == Team.get(255))
                    player255.sendMessage("${player.name}:"+msg)
            }
            null
        } else msg
    }
                ...
                text += buildString {
                    ...
                    if (badplayer.count() <= goodplayer.count()) {
                        if (badplayer.count() != 0) {
                            if (p in badplayer) {
                                appendLine("[red]你是猎杀者你的队友有${badplayernumber}")
                            ...
                            appendLine("[red]该插件采用了大量列表非正常结束游戏可能会造成插件数据异常DW")
