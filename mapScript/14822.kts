...
import wayzer.user.NotificationEntity.T.broadcast
...
class NewGroundAi: GroundAI() {
    ...
    override fun target(x: Float, y: Float, range: Float, air: Boolean, ground: Boolean): Teamc {
        return Units.closestTarget(unit.team, x, y, range,
            { u: mindustry.gen.Unit -> u.checkTarget(air, ground) }
        ) { t: Building -> ground && !(t.block is LaunchPad || t.block == Blocks.basicAssemblerModule) }
    }
}
//循环结构
//坦克火箭弹
        ...
        loop(Dispatchers.game){
            ...
            for(team in teams){
                if(getteamlifes(team) < -20){
                    for(b in Groups.build){
                        if(b.block() is CoreBlock && b.team() == team && b.block() == Blocks.coreAcropolis){
                            var tile = Vars.world.tiles.getn(
                                b.x.toInt() / 8,
                                b.y.toInt() / 8
                            )
...
onEnable{
    launch{
        ...
        if(teams.size < 2){ Groups.player.forEach { p ->
            p.sendMessage("玩家数量不足")
        }; delay(1000) ;return@launch }
        for (team in teams) {
