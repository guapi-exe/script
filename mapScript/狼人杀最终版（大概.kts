...
fun voteStrat(){
    launch(Dispatchers.game){
        ...
        while(true){
            for (allplayer in Groups.player) {
                if(Time.millis() - lastTime >=40_000) return@launch
                ...
                allplayer.unit().apply(StatusEffects.unmoving,100f)
                delay(200)
            }
        }
    }
...
onEnable{
    launch{
        ...
        loop(Dispatchers.game) {
            ...
            while (true) {
                Groups.player.forEach {allplayer->
                    Call.setHudText(allplayer.con, "[yellow]点击自己报警以开始投票\n[green]距离投票开始还有:${(400 * 1000 - (Time.millis() - lastTime)) / 1000}/s")
                    for(allplayer in Groups.player) {
    ...
    launch {
        ...
        loop(Dispatchers.game) {
            ...
            while (true) {
                ...
                Groups.player.forEach { allplayer ->
                    val badplayercount = (Groups.player.toList().count() / 2).toInt()
