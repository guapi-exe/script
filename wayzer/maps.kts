...
val mapheat: ObjectIntMap<String> = ObjectIntMap()
val previousmap: String = ""
val servermap = ArrayList<String>()
val heatlist = ArrayList<String>()
...
fun heatnum(name: String): Int {
    return name.substring(name.lastIndexOf(":") , name.length).toInt()
}
listen<EventType.PlayEvent>{
    val mapid = MapRegistry.nextMapInfo(MapManager.current).id.toString()
    if(mapid !in servermap) { servermap.add(mapid) }
    mapheat.put(mapid,mapheat.get(mapid) + 1)
    heatlist.clear()
    RankingList.clear()
    for(map in servermap){
        val heat = map+":"+mapheat.get(map).toString()
        heatlist.add(heat)
    }
    var heatmax: String = heatlist[0]
    var i = 1
    if(heatlist.size < 3 ){ broadcast("地图不足".with()) ;return@listen }
    while(i<=3) {
        for (heat in heatlist) {
            if (heatnum(heatmax) <= heatnum(heat)) {
                heatmax = heat
            }
        }
        RankingList.add(heatmax)
        heatlist.remove(heatmax)
        i++
    }
    broadcast("$RankingList\n$heatlist".with())
}
registerVar("scoreBroad.ext.contentsVersion", "榜单", DynamicVar.v {
    "1,2,3\n$RankingList"
})
onEnable{
    loop {
        if(LocalDateTime.now().hour in 1..6){
            val mapheatfile = File("/root/config/maps/mapheat.txt")
            if(!mapheatfile.isFile){ mapheatfile.createNewFile() }
            mapheatfile.appendText("${LocalDateTime.now()}|榜单$RankingList\n总$heatlist")
            RankingList.clear()
            heatlist.clear()
            for (map in servermap){
                mapheat.put(map,0)
