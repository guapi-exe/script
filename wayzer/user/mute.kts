...
command("mute", "管理指令: 禁言") {
    ...
    body {
        ...
        if (players.add(uuid)) {
            ...
            history.appendText("${LocalDateTime.now()}|${this.player?.name ?:"控制台"}禁言玩家:${target?.name}\n")
            ...
            val history = File("/root/config/scripts/adminhistory.txt")
            if(!history.isFile){ history.createNewFile() }
            history.appendText("${LocalDateTime.now()}|${this.player?.name ?:"控制台"}解除禁言玩家:${target?.name}\n")
